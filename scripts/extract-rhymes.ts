#!/usr/bin/env bun
import { extractRhymeLetter } from '@qafiyah/db';
...
async function runExtraction(): Promise<void> {
  // Use the postgres driver directly. @qafiyah/db wraps Drizzle but we want
  // raw batch UPDATEs here, not Drizzle's per-row update.
  // biome-ignore lint/suspicious/noExplicitAny: dynamic import from transitive dep
  const postgres = (await import('postgres')).default as any;
  // biome-ignore lint/suspicious/noExplicitAny: dynamic import from transitive dep
  const sql: any = postgres(DB_URL, { max: 4, prepare: false });

  try {
    const rhymeRows = (await sql<RhymeRow[]>`
    ...
    while (true) {
      ...
      if (updates.length > 0) {
        await sql`
          ...
          FROM (VALUES ${sql(updates.map((u) => [u.poem_id, u.rhyme_id]))})
