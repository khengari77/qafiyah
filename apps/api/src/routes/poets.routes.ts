...
const app = new Hono<AppContext>()
  .get('/page/:page', zValidator('param', getPoetsRequestSchema), async (c) => {
    ...
    return c.json(createValidatedResponse('poetsList', responseData, paginationMeta));
  })
  ...
  .get('/:slug/page/:page', zValidator('param', getPoetPoemsRequestSchema), async (c) => {
    ...
    return c.json(createValidatedResponse('poetPoems', responseData, paginationMeta));
  })
  //! ERR HANDLING ------------------------------------------>
  .onError((error, c) => {
    console.error(error);
