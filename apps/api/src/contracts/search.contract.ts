...
export const search = pub
  ...
  .input(
    z.object({
      ...
      match_type: z.string().optional().default('all'),
      meter_ids: z.string().optional(),
      era_ids: z.string().optional(),
      rhyme_ids: z.string().optional(),
      theme_ids: z.string().optional(),
    })
  )
  .errors({
    EMPTY_QUERY: { status: 400, message: 'لا نقل إلا الحروف العربية' },
  })
  .handler(async ({ context, input, errors }): Promise<SearchResponse> => {
