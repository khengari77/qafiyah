...
export function NavLinks({ links, className = "", isMobile = false, onLinkClick }: NavLinksProps) {
  ...
  return (
    <nav aria-label={isMobile ? "القائمة الرئيسية (موبايل)" : "القائمة الرئيسية"} className={className}>
      <ul
        className={
          isMobile
      ...
      >
        {links.map((link) => (
          <li key={link.href} className={isMobile ? "w-full text-right border-b border-zinc-100 pb-2" : ""}>
            <Link
              href={link.href}
              className={`${
                isMobile
                ...
                isActive(link.href) ? (isMobile ? "text-zinc-900 font-bold underline" : "text-zinc-900 underline") : ""
              }`}
