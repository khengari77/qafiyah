import { SITE_URL } from './GLOBALS';

export const htmlHeadMetadata = {
  title: 'موقع قافية | مرجع الشعر العربي',
  description:
    'موقع غير ربحيّ مفتوح المصدر، يُعنى بجمع الشعر العربي وتيسير الوصول إليه. فحفظ شعر العرب من حفظ القرآن، كما قال ابن عباس -رضي الله عنهما-: «الشعر ديوان العرب، فإذا خفي علينا الحرف من القرآن، رجعنا إلى ديوانهم فالتمسناه فيه»',
  keywords:
    'شعر، شعر عربي، قصائد، غزل، معلقات، شعر جاهلي، شعر حديث، قافية، ديوان العرب، أدب، قصائد مشهورة، شعر نبطي',
  openGraphUrl: `${SITE_URL}/opengraph.png`,
  twitterSummaryCardImageUrl: `${SITE_URL}/twitter-summary-card-white.png`,
  poetName: 'قافية',
  author: 'قافية',
  articleSection: 'شعر',
  themeColorHexCode: '#fafafa',
};
